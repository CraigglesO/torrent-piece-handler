...
test("Upload Block", (t) => {
    ...
    tph.prepareUpload(0, 0, 16384, (up) => {
        t.equal(up.slice(0, 9).toString("hex"), buffer_1.Buffer.from([0x00, 0x00, 0x40, 0x09, 0x07, 0x00, 0x00, 0x00, 0x00]).toString("hex"));
...
test("Upload from two files", (t) => {
    ...
    tph.prepareUpload(15, 0, 16384, (up) => {
        t.equal(up.slice(0, 9).toString("hex"), buffer_1.Buffer.from([0x00, 0x00, 0x40, 0x09, 0x07, 0x00, 0x0f, 0x00, 0x00]).toString("hex"));
...
test("Upload from a piece inside file 2", (t) => {
    ...
    tph.prepareUpload(16, 0, 16384, (up) => {
        t.equal(up.slice(0, 9).toString("hex"), buffer_1.Buffer.from([0x00, 0x00, 0x40, 0x09, 0x07, 0x00, 0x10, 0x00, 0x00]).toString("hex"));
        t.equal(up.slice(9).toString("hex"), r.toString("hex"), "check the block");
        t.end();
    });
});
